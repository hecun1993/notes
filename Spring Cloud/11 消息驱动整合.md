...
### 改造 user-service-client 变成消息发送源（Kafka 原生 API）
...
public class User implements Serializable {
    ...
    //序列化只针对非transient字段和非static字段有效
    private Long id;
    private String name;
...
# 生产者 值的序列化
spring.kafka.producer.valueSerializer =com.segumentfault.spring.cloud.lesson12.user.service.client.serializer.ObjectSerialize
```
...
public interface UserMessage {
    ...
    @Input(INPUT)
    // 管道名称默认是本接口的方法名称, 可以在注解中指定
    SubscribableChannel input();
}
```

#### 激活用户消息 Stream 接口(启动类上加@EnableBinding(UserMessage.class))
...
public class UserMessageService {
    ...
    @PostConstruct
    public void init() {
		//MessageStream是一个接口, spring会为其做动态代理, 在另外一个类(本类)中注入其动态代理之后的实现类, 为了我们来控制一下这个实现类, 就写一个init方法, 加上@PostConstruct注解
        
        //1. 最原始的写法, 后期会提取到上面的saveUser方法中
        ...
        //根据contentType来判断, 接收到的数据是字节数组, 还是String
...
> 从上述实现中也可以看出, 对Kafka的依赖很低, 所以可以很方便的换成别的MQ



### 改造 user-service-client 变成消息发送源（ Stream Binder : Rabbit MQ）
...
```xml
<!-- 依赖 Spring Cloud Stream Binder Kafka &ndash;&gt;-->
<!--<dependency>-->
<!--<groupId>org.springframework.cloud</groupId>-->
<!--<artifactId>spring-cloud-stream-binder-kafka</artifactId>-->
<!--</dependency>-->
